...
def extractFromJournal(journal):
    pubIDs = [];
    moreResults = True
    start = 0
    while moreResults:
    ...
    return retrievePub(pubIDs)



def extractFromFile(input='data.csv', output=None):
    # read data from .csv
    pubIDs = []
    ...
    results = retrievePub(pubIDs, pub_dict)
...
def retrievePub(pubIDs, classifiedPubs=[]):
    ...
    for data in xml_data:
        ...
        if obj['abstract'] == '' or full_info_count < 4:
            obj['abstract'] = extractAbstract(obj['doi'])
        results.append(obj)
